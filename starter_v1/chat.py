...
from versioning import artifact_version_dict, build_artifact_version, render_system_prompt


ROOT = Path(__file__).parent
ARTIFACTS_DIR = ROOT / "artifacts"
DEFAULT_MAX_TOOL_ROUNDS = 4
LEGAL_MAX_TOOL_ROUNDS = 12
load_lab_env(ROOT)


def recommended_max_tool_rounds(declarations: list[dict[str, Any]]) -> int:
    """Allow the sequential legal evidence gates without changing other versions."""
    names = {item.get("name") for item in declarations}
    return LEGAL_MAX_TOOL_ROUNDS if "validate_citation" in names else DEFAULT_MAX_TOOL_ROUNDS
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum orchestration rounds. Defaults to 12 for Legal v1 and 4 otherwise.",
    )
    args = parser.parse_args()

    system_prompt = render_system_prompt(args.system_prompt.read_text(encoding="utf-8"))
    tool_declarations = load_tool_declarations(args.tools)
    openai_tools = to_openai_tools(tool_declarations)
    max_tool_rounds = (
        args.max_tool_rounds
        if args.max_tool_rounds is not None
        else recommended_max_tool_rounds(tool_declarations)
    )
